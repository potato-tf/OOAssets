...
--this is identical to parachute_auto_deploy with the values swapped for Reclaimed Access Control on Race Condition
...
function CheckGround(_, activator)
	...
	check = timer.Create(0.015, function()
		...
		Zvalue = downTraceTable["StartPos"] - downTraceTable["HitPos"] - 68
			if Zvalue[3] >= 500 then --increase this to make the parachute deploy when the bot is higher
